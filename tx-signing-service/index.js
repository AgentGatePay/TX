...
// 🔒 SECURITY: Read wallet private key from SECRET FILE
...
try {
    ...
    if (fs.existsSync(secretPath)) {
        ...
        // Fallback: Try to read from root directory (for local development)
        ...
        if (fs.existsSync(localPath)) {
            ...
            throw new Error('Wallet private key file not found');
        }
    }
} catch (error) {
    console.error('❌ CRITICAL: Failed to load wallet private key!');
    console.error('   Expected location: /etc/secrets/wallet-private-key');
    console.error('   Error:', error.message);
    console.error('');
    console.error('📖 Setup Instructions:');
    console.error('   1. In Render dashboard, go to "Secret Files"');
    console.error('   2. Add secret file named: wallet-private-key');
    console.error('   3. Contents: Your wallet private key (0x...)');
...
// 🔒 SECURITY: Read AgentGatePay API key from SECRET FILE
...
try {
    ...
    if (fs.existsSync(secretPath)) {
        ...
        // Fallback: Try environment variable (backward compatibility)
        AGENTGATEPAY_API_KEY = process.env.AGENTGATEPAY_API_KEY;
        if (AGENTGATEPAY_API_KEY) {
            console.log('⚠️  AgentGatePay API key loaded from environment variable (consider using Secret File)');
        } else {
            // Fallback: Try to read from root directory (for local development)
            ...
            if (fs.existsSync(localPath)) {
                ...
                throw new Error('AgentGatePay API key not found');
            }
        }
    }
} catch (error) {
    console.error('❌ CRITICAL: Failed to load AgentGatePay API key!');
    console.error('   Expected location: /etc/secrets/agentgatepay-api-key');
    console.error('   Error:', error.message);
    console.error('');
    console.error('📖 Setup Instructions:');
    console.error('   1. In Render dashboard, go to "Secret Files"');
    console.error('   2. Add secret file named: agentgatepay-api-key');
    console.error('   3. Contents: Your AgentGatePay API key (pk_live_...)');
    console.error('   4. Save and redeploy');
    console.error('');
    process.exit(1);
}
